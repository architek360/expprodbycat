 ...
 */
class SLab_ExpProdByCat_Model_Observer extends Mage_Core_Model_Abstract
{
    ...
    public static function getBlockObserver($key)
    {
        if (self::$_observers === null) {
            $cacheId = 'SLV_CONFIG_BLOCK_OBSERVERS_ExpProdByCat';
    ...
    {
        $cacheId = 'SLV_ExpProdByCat_CONFIG_BLOCK_OBSERVERS_KEYS';
        $cacheData = Mage::app()->getCache()->load($cacheId);
        if ($cacheData !== false) {
            $result = unserialize($cacheData);
        } else {
